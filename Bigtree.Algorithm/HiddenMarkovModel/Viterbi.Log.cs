using Bigtree.Algorithm.HiddenMarkovModel.Helpers;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Bigtree.Algorithm.HiddenMarkovModel
{
    public partial class Viterbi
    {
        public int[] LogForward(double[,] logA, double[,] logB, double[] logPi, int[] observations)
        {
            double logLikelihood = 0;
            return LogForward(logA, logB, logPi, observations, out logLikelihood);
        }

        public static int[] LogForward(double[,] logA, double[,] logB, double[] logPi, int[] observations, out double logLikelihood)
        {
            ...
            for (int i = 0; i < N; ++i)
            {
                fwd[0, i] = logPi[i] + logB[i, observations[0]];
            }
            ...
            for (int t = 1; t < T; ++t)
            {
                for (int i = 0; i < N; ++i)
                {
                    ...
                    fwd[t, i] = maxWeight + logB[i, observations[t]];
                    V[t, i] = maxState;
                }
            }
